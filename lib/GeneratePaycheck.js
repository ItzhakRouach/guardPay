...
export const handleGeneratePDF = async (
  ...
  shifts.forEach((s) => {
    ...
    const isHoliday = s.is_holiday === true;
    ...
    const hourTypes = [
      ...
      {
        key: "h150s",
        label: isHoliday ? "שעות חג 150%" : "שעות שבת 150%",
      ...
      {
        key: "h175s",
        label: isHoliday ? "שעות נוספות חג 175%" : "שעות נוספות שבת 175%",
        hours: Number(s.h175_extra_hours),
        factor: 1.75,
      },
      {
        key: "h200s",
        label: isHoliday ? "שעות נוספות חג 200%" : "שעות נוספות שבת 200%",
    ...
    hourTypes.forEach((type) => {
      if (type.hours > 0) {
        ...
        // המפתח לקבוצה כולל את הלייבל כדי להפריד בין "שבת" ל"חג" גם אם התעריף זהה
  ...
  // רכיבי הזיכויים (בצבע ירוק)
  const settlementCreditRow =
    monthlyReport?.settlementBenefitValue > 0
      ? `
    <tr>
      <td style="color: #2e7d32;">זיכוי יישוב מוטב (${profile.settlement_percent}%)</td>
      <td style="color: #2e7d32;">- ${monthlyReport.settlementBenefitValue.toFixed(2)}</td>
    </tr>
  `
      : "";
  ...
  const creditPointsRow = `
    <tr>
      ...
      <td style="color: #2e7d32;">- ${(profile.credit_points * 242 || 544.5).toFixed(2)}</td>
    </tr>
  `;
  ...
  const htmlStyles = `
    <style>
      ...
      .section-title { font-size: 18px; color: #333; border-right: 4px solid #e94560; padding-right: 10px; margin-top: 30px; margin-bottom: 10px; }
  ...
  const htmlContent = `
    ...
    <body>
      <div class="header-container">
        ...
        <div class="header-row"><span class="label">כמות שעות:</span><span>${totals.totalHours}</span></div>
      ...
      <table class="payroll-table">
        <tbody>
          ${creditPointsRow}
          ${settlementCreditRow}
          <tr><td>מס הכנסה (סופי)</td><td>${(monthlyReport?.incomeTax || 0).toFixed(2)}</td></tr>
          <tr><td>ביטוח לאומי ובריאות</td><td>${(monthlyReport?.bituahLeumiAndHealth || 0).toFixed(2)}</td></tr>
          <tr><td>פנסיה</td><td>${(monthlyReport?.pensia || 0).toFixed(2)}</td></tr>
        </tbody>
      </table>
