...
export const handleGeneratePDF = async (
  ...
  const htmlContent = `
    ...
    <body>
      ...
      <table class="payroll-table">
        <tbody>
          <tr><td>מס הכנסה</td><td>${(monthlyReport?.incomeTax || 0).toFixed(
            2,
          )}</td></tr>
          <tr><td>ביטוח לאומי ובריאות</td><td>${(
            monthlyReport?.bituahLeumiAndHealth || 0
          ).toFixed(2)}</td></tr>
          <tr><td>פנסיה</td><td>${(monthlyReport?.pensia || 0).toFixed(2)}</td></tr>
        </tbody>
      </table>
      <div class="summary-section">
        <div class="summary-row"><span>סה"כ ברוטו:</span><span>₪ ${(
          monthlyReport?.bruto || 0
        ).toFixed(2)}</span></div>
        <div class="summary-row net"><span>נטו לתשלום:</span><span class="summary-value">₪ ${(
          monthlyReport?.neto || 0
        ).toFixed(2)}</span></div>
      </div>
    </body>
    </html>
