...
const HOUR_TYPES = [
  ...
  { key: "h125_extra_hours", label: "ש.נ מוקדן 125%", factor: 1.25 },
  { key: "h150_extra_hours", label: "ש.נ מוקדן 150%", factor: 1.5 },
  { key: "h150_shabat", label: "שעות שבת 150%", factor: 1.5 },
  { key: "h175_shabat", label: "ש.נ שבת 175%", factor: 1.75 },
  { key: "h200_shabat", label: "ש.נ שבת 200%", factor: 2 },
  { key: "h150_holiday", label: "שעות חג 150%", factor: 1.5 },
  { key: "h175_holiday", label: "ש.נ חג 175%", factor: 1.75 },
  { key: "h200_holiday", label: "ש.נ חג 200%", factor: 2 },
];
...
const aggregateEarnings = (shifts, profile) => {
  ...
  shifts.forEach((s) => {
    ...
    HOUR_TYPES.forEach((type) => {
      ...
      const key = `${type.label}_${finalRate.toFixed(2)}`;
      if (!groups[key]) {
        groups[key] = { label: type.label, hours: 0, rate: finalRate, amount: 0 };
      }
...
export const handleGeneratePDF = async (
  ...
  const settlementBenefit = Number(monthlyReport?.settlementBenefit || 0);
