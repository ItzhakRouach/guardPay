...
export const handleGeneratePDF = async (
  ...
  shifts.forEach((s) => {
    ...
    // Special-pay hours (150%/175%/200%) are stored in the same fields for
    // weekend and holiday work — the `is_holiday` flag on the shift decides
    // how we label them in the paycheck.
    const hourTypes = [
  ...
  const rows = Object.values(groupedRows)
    ...
    <tr>
      ...
      <td>₪${(group.hours * Number(group.rate)).toFixed(2)}</td>
    </tr>
  `,
    )
    .join("");
  ...
  const settlementCreditRow =
    ...
    <tr>
      ...
      <td style="color: #2e7d32; font-weight: bold;">- ₪${monthlyReport.settlementBenefitValue.toFixed(2)}</td>
    </tr>
  `
      : "";
